...
export default function RainbowHerbicides({ years, series }: Props) {
  ...
  // Accumulation (running cumulative total, default) vs Each year.
  const [mode, setMode] = useState<'cum' | 'year'>('cum')

  // In cumulative mode each series' value at year i is its running total up to
  // year i, so the stacked area climbs to the grand total by 1971.
  const plot =
    mode === 'cum'
      ? series.map((s) => {
          let run = 0
  ...
  const activeSeries = series.find((s) => s.key === sel) // card total is always the full-period sum
  // ~5 nice gridline steps whatever the scale (6M yearly → 1M; 20M cumulative → 5M).
          ...
          <div className="rainbow-main">
            ...
            <div className="rainbow-chart-top">
              ...
              <div className="rainbow-mode" role="tablist" aria-label="Chart mode">
                <button
                  role="tab"
                  aria-selected={mode === 'cum'}
                  className={`rainbow-mode-btn${mode === 'cum' ? ' is-active' : ''}`}
                  onClick={() => setMode('cum')}
                >
                  Accumulation
                </button>
                <button
                  ...
                  className={`rainbow-mode-btn${mode === 'year' ? ' is-active' : ''}`}
            ...
            <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={RAINBOW.chartTitle} className="rainbow-svg">
              ...
              {/* axes: y (vertical) + x baseline */}
              <line x1={M.left} x2={M.left} y1={M.top} y2={M.top + PH} className="rainbow-axis-line" />
              <line x1={M.left} x2={W - M.right} y1={M.top + PH} y2={M.top + PH} className="rainbow-axis-line" />
              {/* stacked bands (smoothed, gradient-filled, no outline) */}
              {bands.map((b) => {
                const dim = sel !== b.key
                return (
                  <path
              ...
              {/* x labels — all years written in full */}
              {years.map((yr, i) => (
