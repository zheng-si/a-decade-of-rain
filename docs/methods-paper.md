...
Spreading along the run assumes a constant rate, which is an assumption. So we push it as hard as the record allows. The one thing the record does establish is that the recorded volume was released somewhere along the track the aircraft flew; within that constraint we vary the rate profile to extremes and measure how far the field moves relative to the constant-rate field.
...
Why a field at all, when the Atlas draws the record's own marks: the Story's eight nodes sit between zoom 6.3 and 9.6 on a map that also carries relief, place names, the narrative card and a playhead, and at those zooms the marks are either a lattice of cell-bound dots or runs a few pixels long. The field is the fine-tier binning smoothed over about two cells, so that where the decade's volume accumulated reads at a glance and fills in month by month as the reader scrolls. It is a reading aid over the same gallons rather than a further quantity; its scale is ordinal (the legend says less and more, and the key's note says what is counted); and nothing in it claims deposition. Three alternatives were drawn on the page before this was settled, in September 2026: the Atlas's dots at their own size and at two and a half to four and a half times it, the same dots soft-edged, and the Stellmans' hit grid, which went first because the shipped table carries no time. The dots at the Atlas's size did not carry the page; enlarged and softened they approached the field at the country zoom but not in the valleys, where the kernel does the filling. The field stayed, with its constants named here.
